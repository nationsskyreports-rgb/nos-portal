...
async function loadRecordsMyLog() {
  ...
  const wrap  = document.getElementById('records-mylog-content');
  if (!wrap || !agent) return;

  const today = getLocalDateStr();
  const monthStart = today.slice(0, 7) + '-01';

  wrap.innerHTML = '<div class="empty-state"><i class="fas fa-spinner spinner"></i> Loading...</div>';
  ...
  try {
    ...
    const [calls, wasps, rems] = await Promise.all([
      ...
      fetch(`${SB_URL_SCH}/rest/v1/call_reminders?agent_name=eq.${encodeURIComponent(agent)}&is_done=eq.false&order=reminder_date,reminder_time`, { headers }).then(r => r.json()),
    ]);
    ...
    // Reminders section
    if ((rems||[]).length) {
      html += `<div style="background:var(--surface);border:1px solid rgba(245,158,11,.3);border-radius:14px;padding:16px;margin-bottom:16px;">
        <div style="font-size:13px;font-weight:800;color:#F59E0B;margin-bottom:10px;"><i class="fas fa-bell"></i> My Reminders (${rems.length})</div>`;
      html += (rems||[]).map(r => {
        ...
        const label   = isPast ? '🔴 Overdue' : isToday ? '🟡 Today' : r.reminder_date;
        return `<div style="display:flex;align-items:center;gap:10px;padding:10px;border:1px solid ${border};border-radius:10px;margin-bottom:6px;background:var(--surface);">
          <div style="flex:1;font-size:12px;"><span style="font-weight:700;">${r.customer_name||'—'}</span> <span style="color:var(--muted);font-family:monospace;">${r.customer_mobile||''}</span>
            <div style="font-size:11px;color:var(--muted);margin-top:2px;"><span style="font-weight:700;${isPast?'color:var(--danger);':isToday?'color:var(--primary);':''}">${label}</span>${r.reminder_time ? ' · '+r.reminder_time.substring(0,5) : ''}${r.note ? ' · '+r.note : ''}</div>
          </div>
          <button onclick="markReminderDone('${r.id}').then(()=>loadRecordsMyLog())" style="background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.3);border-radius:8px;padding:6px 10px;font-size:11px;font-weight:700;color:#10B981;cursor:pointer;">✓ Done</button>
        </div>`;
      }).join('');
      html += '</div>';
    }
    ...
    // Recent calls
    html += `<div style="font-size:13px;font-weight:800;color:var(--text);margin-bottom:10px;"><i class="fas fa-history" style="color:var(--primary);margin-right:6px;"></i> Recent Calls (${data.length} this month)</div>`;
    if (data.length) {
      html += data.slice(0, 20).map(c => {
        ...
        return `<div style="display:flex;align-items:center;gap:10px;padding:10px;border-bottom:1px solid var(--border);font-size:12px;">
          <span>${icon}</span>
          <span style="min-width:80px;color:var(--muted);font-weight:600;">${date} ${time}</span>
          <span style="font-weight:700;color:var(--text);flex:1;">${c.customer_name||'—'}</span>
          <span style="color:var(--accent,var(--primary));font-weight:600;">${c.project||''}</span>
          <span style="color:var(--muted);">${c.call_reason||''}</span>
        </div>`;
      }).join('');
    } else {
      html += '<div style="text-align:center;color:var(--muted);padding:20px;">No calls this month yet</div>';
    }

    wrap.innerHTML = html;
  } catch(e) {
    wrap.innerHTML = '<div class="empty-state">Connection error</div>';
  }
}

/* ─── SEARCH from Search sub-tab ─── */
async function clSearchCustomer() {
  const input   = document.getElementById('cl-search-input');
  const results = document.getElementById('cl-search-results');
  const query   = input?.value?.trim();
  if (!query || query.length < 2) { if(results) results.innerHTML = '<div style="font-size:12px;color:var(--muted);padding:8px;">Type at least 2 characters</div>'; return; }

  results.innerHTML = '<div style="font-size:12px;color:var(--muted);padding:8px;"><i class="fas fa-spinner spinner"></i> Searching...</div>';

  try {
    const headers = { 'apikey': SB_KEY_SCH, 'Authorization': `Bearer ${window._authToken || SB_KEY_SCH}` };
    const filter = `or=(customer_name.ilike.%${encodeURIComponent(query)}%,customer_mobile.ilike.%${encodeURIComponent(query)}%)&order=logged_at.desc&limit=20`;
    const [calls, wasps] = await Promise.all([
      fetch(`${SB_URL_SCH}/rest/v1/call_logs?${filter}`, { headers }).then(r => r.json()),
      fetch(`${SB_URL_SCH}/rest/v1/whatsapp_logs?${filter}`, { headers }).then(r => r.json()),
    ]);
    const all = [...(calls||[]).map(r=>({...r,_s:'📞'})), ...(wasps||[]).map(r=>({...r,_s:'💬'}))].sort((a,b) => new Date(b.logged_at) - new Date(a.logged_at));

    if (!all.length) { results.innerHTML = '<div style="padding:12px;color:var(--muted);font-size:13px;">No results found</div>'; return; }

    results.innerHTML = `<div style="font-size:11px;color:var(--muted);margin-bottom:8px;">${all.length} result(s)</div>` +
      all.map(c => {
        const date = c.logged_at ? new Date(c.logged_at).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}) : '';
        return `<div style="display:flex;align-items:center;gap:10px;padding:10px;border-bottom:1px solid var(--border);font-size:12px;">
          <span>${c._s}</span>
          <span style="min-width:85px;color:var(--muted);font-weight:600;">${date}</span>
          <span style="font-weight:700;flex:1;">${c.customer_name||'—'}</span>
          <span style="font-family:monospace;color:var(--muted);">${c.customer_mobile||''}</span>
          <span style="color:var(--accent,var(--primary));">${c.project||''}</span>
          <span style="color:var(--muted);">${c.call_reason||''}</span>
          <span style="color:var(--muted);">${c.agent_name||''}</span>
        </div>`;
      }).join('');
  } catch(e) { results.innerHTML = '<div style="color:var(--danger);font-size:12px;">Error searching</div>'; }
}
...
function waitForAgentThenLoadDashboard(attempts) {
  ...
  if (agent && agent !== '' && agent !== 'Agent') {
    ...
    // Retry every second for up to 15 seconds
    setTimeout(() => waitForAgentThenLoadDashboard(attempts + 1), 1000);
  }
}
