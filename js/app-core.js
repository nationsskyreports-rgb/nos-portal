...
function openChangePassword() {
  ...
  if (infoEl) infoEl.innerText = '👤 ' + userName;
...
async function submitChangePassword() {
  ...
  if (!newP || !confP)  { msg.style.color = 'var(--danger)'; msg.innerText = 'Fill all fields'; return; }
  if (newP.length < 6)  { msg.style.color = 'var(--danger)'; msg.innerText = 'Password must be at least 6 characters'; return; }
  ...
  try {
    ...
    // Update password directly — no old password needed when session is active
    const { error: updateErr } = await sbClient.auth.updateUser({ password: newP });
    if (updateErr) { msg.style.color = 'var(--danger)'; msg.innerText = 'Update failed: ' + updateErr.message; return; }
...
function toggleReset(show) {
  ...
  if (show) {
    if (!agentName) { customAlert('Error', 'Select your name first'); return; }
    loginForm.classList.add('hidden');
    resetForm.classList.remove('hidden');
    document.getElementById('reset-agent-name').innerText = agentName;
    document.getElementById('reset-msg').innerText = '';
    fetch(`${SB_URL_SCH}/rest/v1/agents?select=email&formal_name=eq.${encodeURIComponent(agentName)}&limit=1`,
      ...
      .then(r => r.json()).then(data => {
        ...
        if (email) {
          ...
          preview.innerHTML = '📧 ' + masked;
          preview.style.display = 'block';
          preview.style.color = '#60a5fa';
          preview.style.borderColor = 'rgba(59,130,246,.2)';
          preview.style.background = 'rgba(59,130,246,.08)';
        } else {
          preview.innerHTML = '⚠️ No email found — contact admin';
          preview.style.display = 'block';
          preview.style.color = '#f87171';
          preview.style.borderColor = 'rgba(239,68,68,.2)';
          preview.style.background = 'rgba(239,68,68,.08)';
        }
      });
  } else {
...
async function sendResetEmail() {
  ...
  if (!name) { msg.style.color = '#f87171'; msg.innerText = 'Select your name first'; return; }
  ...
  try {
    const res = await fetch(
      `${SB_URL_SCH}/rest/v1/agents?select=email&formal_name=eq.${encodeURIComponent(name)}&limit=1`,
      { headers: { 'apikey': SB_KEY_SCH, 'Authorization': `Bearer ${SB_KEY_SCH}` } }
    );
    const data = await res.json();
    const email = data?.[0]?.email;
    if (!email) {
      ...
      msg.innerText = 'No email found. Contact admin.';
      btn.disabled = false; btn.innerHTML = '<i class="fas fa-envelope"></i> Send Reset Link';
      return;
    }
    ...
    if (error) {
      ...
      msg.innerHTML = '✅ Reset link sent! Check your email.<br><span style="font-size:11px;color:#94a3b8;">Check spam folder if you don\'t see it.</span>';
